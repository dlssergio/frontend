<script setup>
/**
 * OrdenCompraCreate.vue — Orden de Compra a Proveedor
 * Layout fijo tipo POS. Sin axios hardcodeado.
 * Al confirmar genera stock RCPT (A Recibir).
 * Desde el detalle se puede generar un Remito a partir de esta OC.
 */
...
const origenId = route.query.origen ?? null   // para "OC a partir de otra"
...
const measureLayout = () => {
  ...
  layoutRaf = requestAnimationFrame(() => {
    ...
    try {
      ...
      posRootEl.value.style.setProperty('--oc-h',
...
const cargarOrigenComprobante = async () => {
  try {
    ...
    if (p) { proveedorId.value = p.id; proveedorNombre.value = p.razon_social; proveedorCuit.value = p.cuit ?? '' }
    if (res.data.items?.length) {
      items.value = res.data.items.map(i => ({
        key: Date.now() + Math.random(), articuloId: i.articulo.id,
        busqueda: i.articulo.cod_articulo, descripcion: i.articulo.descripcion,
        cantidad: parseFloat(i.cantidad), costo: parseFloat(i.costo_unitario || 0),
      }))
    }
...
const columns = [
  ...
  { title: 'Precio Est.',  dataIndex: 'costo',       width: 140, align: 'right' },
...
const cargarAuxiliares = async () => {
  ...
  try {
    ...
    if (tR.status === 'fulfilled') {
      // OC = clase C + NO afecta stock físico (o nombre contiene "orden")
      tiposComp.value = (tR.value.data.results ?? tR.value.data)
        .filter(t => t.clase === 'C' && (
          !t.afecta_stock_fisico ||
...
const guardar = async (confirmar = false) => {
  ...
  try {
    ...
    if (confirmar && res.data.id) {
      ...
      message.success('Orden de Compra confirmada — stock A Recibir actualizado.')
    } else {
      message.success('Orden de Compra guardada como borrador.')
    }
...
<template>
  <div ref="posRootEl" class="oc-root">
    ...
    <div class="oc-body">
      <section class="left-panel">
        <div class="panel-header">
          ...
          <span class="panel-title"><ShoppingOutlined style="margin-right:6px" />Nueva Orden de Compra</span>
        </div>
        ...
        <div class="panel-block">
          <div class="section-label">Datos de la OC</div>
        ...
        <div class="panel-total"><span class="total-lbl">TOTAL EST.</span><span class="total-val">$ {{ fmtM(subtotal) }}</span></div>
        <div class="panel-actions">
          ...
          <a-button block type="primary" :loading="submitting" @click="guardar(true)"><SaveOutlined /> Confirmar OC</a-button>
        </div>
      </section>
              ...
              <a-table-summary fixed>
                <a-table-summary-row>
                  <a-table-summary-cell :index="0" :col-span="5" align="right">
                    <strong style="font-size:11px;text-transform:uppercase;letter-spacing:.05em;color:var(--text-2)">TOTAL EST.</strong>
...
<style scoped>
.oc-root { --oc-h:100vh; height:var(--oc-h); min-height:var(--oc-h); overflow:hidden; background:var(--surface-0,transparent); }
.oc-body { display:flex; gap:14px; padding:14px; height:100%; overflow:hidden; min-height:0; }
